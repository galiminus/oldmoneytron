export default {
    "leftnav.settings": "Settings",
    "leftnav.buyRights": "Buy the copyrights",

    "variation.list.range.day": "By day",
    "variation.list.range.month": "By month",
    "variation.list.range.year": "By year",

    "variation.list.frequency.0": "one time",
    "variation.list.frequency.1": "every month",
    "variation.list.frequency.3": "every three month",
    "variation.list.frequency.6": "every six month",
    "variation.list.frequency.12": "every year",
    "variation.list.frequency.24": "every two years",

    "settings.form.title": "Settings",
    "settings.form.language": "Language",
    "settings.form.language.fr": "Français",
    "settings.form.language.en": "English",

    "buyRights.title": "Buy the copyrights",

    "variation.form.new": "New entry",

    "variation.form.tab.income": "Income",
    "variation.form.tab.spending": "Spending",
    "variation.form.label": "Origin",
    "variation.form.amount": "Amount",

    "variation.form.frequency.0": "One time",
    "variation.form.frequency.1": "Each month",
    "variation.form.frequency.3": "Every three month",
    "variation.form.frequency.6": "Every sex month",
    "variation.form.frequency.12": "Every year",
    "variation.form.frequency.24": "Every two years"
};
